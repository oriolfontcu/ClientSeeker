...
const BusinessesDataTable = () => {
  ...
  const fetchBusinessesByLocationAndSector = async () => {
    ...
    try {
      ...
      if (process.env.NODE_ENV !== "production"){
        response = await fetch(`/api/getBusinessesByLocationAndSector?location=${location}&sector=${sector}&website=${selectedStatusWeb?.value}&mail=${selectedStatusMail?.value}&limit=${limit}`);
      } else {
        response = await fetch(`/api/fakeCompanies`);
      }
  ...
  return (
    <div>
    <div className="justify-between items-center p-4 rounded-xl w-screen">
